"""
Trains microGPT and exports visualization data as JSON.
Duplicates logic from microgpt.py to avoid its top-level side effects.
Run: python export_data.py
Outputs: frontend/public/data/{weights,training-log,checkpoints,vocab}.json
"""
...
def generate_names(n=8, temperature=0.5):
    names = []
    for _ in range(n):
...
for step in range(num_steps):
    ...
    step_num = step + 1
    print(f"step {step_num:4d} / {num_steps:4d} | loss {loss.data:.4f}")

    if step_num in checkpoint_steps:
        # Save RNG state, generate, restore
        rng_state = random.getstate()
        names = generate_names(8, temperature=0.5)
...
for key in weight_order:
    for row in state_dict[key]:
        for val in row:
            weights.append(round(val.data, 6))

with open(os.path.join(out_dir, 'weights.json'), 'w') as f:
    json.dump(weights, f)

with open(os.path.join(out_dir, 'training-log.json'), 'w') as f:
